    ...
    for v in critical_path:
        ...
        for machine in data['machines'][factory][v]:
            ...
            if nowPop2['index'] != index_v and nowPop2['index'] != -1 and nowPop2['objective'] != 2147483647:
                new_machine = np.copy(machines)
                new_machine[v - 1] = nowPop2['machine']
                # print(nowPop2['objective'])
                node_best = indexes_to_nodes[nowPop2['index']]
                j_b, o_b = tools.get_job_operation(node_best, data)
                if nowPop2['direction'] == 'all':
                    new_makespan = evalPop.evaluate_ind(schedule, new_machine, factories, data)
                    new_schedule = schedule
                elif nowPop2['direction'] == 'forward':
                    ...
                    new_schedule, new_makespan = solveSchedule.backward_schedule(schedule, new_machine, factories, PM_index,
                                                                                 PJ_index, SM_index, SJ_index,
                                                                                 nowPop2['index'], index_v,
                                                                                 data, factory=factory)
                # print(nowPop2['objective'], ' ,', end=' ')
                nowPop2['objective'] = new_makespan[0,1]
                # print(nowPop2['objective'])
                if nowPop2['objective'] < bestPop2['objective']:
                    bestPop2 = nowPop2

        # if bestPop['index'] != index_v and bestPop['index'] != -1:
        #     if len(bestPops) > 0 and bestPop['objective'] == bestPops[0]['objective']:
        #         bestPops.append(bestPop)
        #     else:
        #         bestPops = [bestPop]
        # if bestPop2['index'] != index_v and bestPop2['index'] != -1:
        #     if len(bestPop2s) > 0 and bestPop2['objective'] == bestPop2s[0]['objective']:
