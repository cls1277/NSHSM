    ...
    for v in critical_path:
        ...
        for machine in data['machines'][factory][v]:
            ...
            if machine not in all_operations_of_machine:
                ...
                if job_u == job_v:
                    if operation_u > operation_v:
                        ...
                        node_best = indexes_to_nodes[nowPop['index']]
                        j_b, o_b = tools.get_job_operation(node_best, data)
                        new_schedule, new_makespan = solveSchedule.backward_schedule(schedule, new_machine,
                                                                                     factories,
                                                                                     PM_index,
                                                                                     PJ_index, SM_index, SJ_index,
                                                                                     nowPop['index'], index_v,
                                                                                     data)
                        nowPop['objective'] = new_makespan[0, 0]
                        ...
                        node_best = indexes_to_nodes[nowPop['index']]
                        j_b, o_b = tools.get_job_operation(node_best, data)
                        new_schedule, new_makespan = solveSchedule.forward_schedule(schedule, new_machine,
                                                                                    factories,
                                                                                    PM_index,
                                                                                    PJ_index, SM_index, SJ_index,
                                                                                    index_v,
                ...
                if L < R:
                    ...
                    node_best = indexes_to_nodes[nowPop['index']]
                    j_b, o_b = tools.get_job_operation(node_best, data)
                    new_schedule, new_makespan = solveSchedule.backward_schedule(schedule, new_machine, factories,
                                                                                 PM_index,
                                                                                 PJ_index, SM_index, SJ_index,
                                                                                 nowPop['index'], index_v,
                                                                                 data)
                    obj = new_makespan[0, 0]
                    ...
                    for pos in range(R, L): # from R to L-1
                        ...
                        new_schedule, new_makespan = solveSchedule.forward_schedule(schedule, new_machine,
                                                                                    ...
                                                                                    nowPop['index'],
                                                                                    data)
                        # print(nowPop['objective'], ' ,', end=' ')
                        now_obj = new_makespan[0, 0]
                ...
                for pos in range(R, L+2):
                    ...
                    if (dettime >= time_v_k) or (time_v_origin + dettime >= data['MACHINES'] * (time_v_k - dettime)):
                        ...
                        if (SJ_index[index_v] == -1) or (end_time_index[index_v] + time_v_k <= st_index_copy[SJ_index[index_v]]):
                            new_machine = np.copy(machines)
                            new_machine[v - 1] = machine
                            # print(nowPop2['objective'])
                            node_best = indexes_to_nodes[all_operations_of_machine[machine][pos]]
                            j_b, o_b = tools.get_job_operation(node_best, data)
                            new_schedule, new_makespan = solveSchedule.backward_schedule(schedule, new_machine,
                                                                                         factories, PM_index,
                                                                                         PJ_index, SM_index,
                                                                                         SJ_index,
                                                                                         all_operations_of_machine[machine][pos], index_v,
                                                                                         data, factory=factory)
                            now_obj = new_makespan[0, 1]
            ...
            if nowPop['index'] != index_v and nowPop['index'] != -1:
                ...
                node_best = indexes_to_nodes[nowPop['index']]
                j_b, o_b = tools.get_job_operation(node_best, data)
                if nowPop['direction'] == 'all':
                    new_makespan = evalPop.evaluate_ind(schedule, new_machine, factories, data)
                    new_schedule = schedule
                elif nowPop['direction'] == 'forward':
